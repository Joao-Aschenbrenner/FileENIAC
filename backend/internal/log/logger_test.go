...
func TestWithCorrelationID(t *testing.T) {
	...
	ctx2 := WithCorrelationID(ctx, "abc123")
	id, ok := ctx2.Value(correlationIDKey).(string)
	if !ok {
	...
	if _, ok := ctx.Value(correlationIDKey).(string); ok {
		t.Error("original context must not be modified")
	}
}

func TestWithCorrelationID_EmptyGenerates(t *testing.T) {
	ctx := WithCorrelationID(context.Background(), "")
	if id, ok := ctx.Value(correlationIDKey).(string); !ok || id == "" {
		t.Errorf("expected generated ID, got %q", id)
	}
}
...
func TestWithContext_NoCorrelationID(t *testing.T) {
	...
	old := defaultLogger
	defaultLogger = zap.New(observed)
...
func TestWithContext_WithCorrelationID(t *testing.T) {
	...
	entry := logs.All()[0]
	corrID := entry.ContextMap()["correlation_id"]
	if corrID != "xyz789" {
		t.Errorf("expected correlation_id=xyz789, got %v", corrID)
	}
}
...
func TestWithContext_OriginalUnchanged(t *testing.T) {
	...
	ctx := WithCorrelationID(context.Background(), "id1")
	logger1 := WithContext(ctx)
	ctx2 := WithCorrelationID(context.Background(), "id2")
	logger2 := WithContext(ctx2)
	if logger1 == logger2 {
		t.Error("loggers with different correlation IDs should be different instances")
	}
}
